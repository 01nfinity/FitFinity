...
export default function CalendarScreen() {
  ...
  const loadMarkedDates = async () => {
    try {
      ...
      dates.forEach(d => {
        marks[d.date] = { marked: true, dotColor: theme.accent };
      });
  ...
  const loadDayWorkouts = async (date: string) => {
    setLoading(true);
    try {
      const logs = await db.getAllAsync<WorkoutLog>('SELECT * FROM logs WHERE date = ?', date);
        ...
        {loading ? (
          ...
          dayWorkouts.map((workout, idx) => (
            <View key={workout.id} style={[styles.workoutCard, idx < dayWorkouts.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border }]}>
              ...
              {workout.sentiment && (
                <Text style={[styles.sentimentText, { color: theme.tabIconDefault }]}>
